function test_zmumps()
    ...
    id.ICNTL(1:4) = 1; % Turn off output
    ...
    id.ICNTL(7) = 7; % Force use of METIS
    ...
    % 9. Solve with UMFPACK (umfpack_v1)
    if exist('umfpack_v1', 'file') == 3
        disp('Solving with UMFPACK...');
        opts_umf = umfpack_v1;
        opts_umf.prl = 0;
        tic;
        [x_umfpack, ~] = umfpack_v1(A, '\', b, opts_umf);
        t_umfpack = toc;
        rel_res_umfpack = norm(A * x_umfpack - b) / norm(b);
        fprintf('UMFPACK time: %.4f seconds | Relative residual: %.2e\n', t_umfpack, rel_res_umfpack);
    else
    ...
    % 10. Solve with MATLAB built-in backslash
    disp('Solving with MATLAB A\b...');
    tic;
    x_backslash = A \ b;
    t_backslash = toc;
    rel_res_backslash = norm(A * x_backslash - b) / norm(b);
    fprintf('MATLAB A\\b time: %.4f seconds | Relative residual: %.2e\n', t_backslash, rel_res_backslash);

    % 11. Timing and accuracy summary
    fprintf('\n=== Solver Comparison Summary ===\n');
    fprintf('  ZMUMPS   : %.4f seconds | Relative residual: %.2e\n', t_solve, norm(A*x - b)/norm(b));
    if ~isnan(t_umfpack)
        fprintf('  UMFPACK  : %.4f seconds | Relative residual: %.2e\n', t_umfpack, rel_res_umfpack);
    end
    fprintf('  A\\b      : %.4f seconds | Relative residual: %.2e\n', t_backslash, rel_res_backslash);
